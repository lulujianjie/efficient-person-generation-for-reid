...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # {node:idx_in_unique_nodes_list}

        center_idx = torch.Tensor([unique_nodes_map[center_node], ]).type(torch.long)
        one_hop_idcs = torch.Tensor([unique_nodes_map[_node] for _node in hops[0]]).type(torch.long)
        # 1-hop idx
        ...
        for node in unique_nodes_list:
            ...
            for unn in neighbors:
                if unn in unique_nodes_list:
                    A[unique_nodes_map[node], unique_nodes_map[unn]] = 1
                    A[unique_nodes_map[unn], unique_nodes_map[node]] = 1
...
def make_dataloader(Cfg):
    trainset = Dataset(Cfg.TRAIN_FEATS_PATH,
                       Cfg.TRAIN_KNN_DISTMAT_PATH,
                       Cfg.TRAIN_LABELS_PATH,
                       Cfg.SEED,
                       Cfg.NUM_HOP,
                       Cfg.NUM_ACTIVE_CONNECTION)
    testset = Dataset(Cfg.TEST_FEATS_PATH,
                       Cfg.TEST_KNN_DISTMAT_PATH,
    ...
    trainloader = data.DataLoader(trainset, batch_size=Cfg.BATCHSIZE,
                                  num_workers=Cfg.DATALOADER_NUM_WORKERS, shuffle=True, pin_memory=True)
    testloader = data.DataLoader(testset, batch_size=Cfg.TEST_BATCHSIZE,
                                  num_workers=Cfg.DATALOADER_NUM_WORKERS, shuffle=False, pin_memory=True)
