...
class PersonReIDMAP:
    ...
    def cosine(self, x, y):
        ...
        x = self.normalize(x)
        y = self.normalize(y)
        cosine = np.matmul(x, np.transpose(y))
        return cosine
    ...
    def l2(self, x, y):
        ...
        xx = np.tile(np.power(x, 2).sum(1, keepdims=True), [1, n])
        yy = np.tile(np.power(y, 2).sum(1, keepdims=True), [1, m]).transpose()
        xy = np.matmul(x, y.transpose())
        return np.sqrt(np.maximum(xx - 2*xy + yy, 1e-12)) # for numeric stability
